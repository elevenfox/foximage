...
class foxPlus {	
	...
	public function getSqlFile($path) {
		...
		$arr = Ext_Dir::getDirList(Wee::$config['data_path'] . 'db_backup', Ext_Dir::TYPE_FILE, array(), array('sql'));
		$sqlFile = array();
	...
	public function getSamePreFile($path, $filename) {
		...
		if (preg_match("/^(\d+_\d+)_(\d+)\.sql$/i", basename($filename), $tmp)) {
			$allFiles = Ext_Dir::getDirList(Wee::$config['data_path'] . 'db_backup', Ext_Dir::TYPE_FILE, array(), array('sql'));
			if ($allFiles) {
