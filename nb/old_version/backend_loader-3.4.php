...
if (!defined('WRITABLE_PATH')) {
    define('WRITABLE_PATH', __DIR__ . '/../cache/neverblock');
}
...
if (!isset($testRun)) {
    ...
    $cookie = (!empty($_COOKIE[ADS_COOKIE_NAME])) ? json_decode($_COOKIE[ADS_COOKIE_NAME], true) : array();
    if (!is_array($cookie)) {
        $cookie = array();
